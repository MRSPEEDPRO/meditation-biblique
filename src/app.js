...
(function () {
  ...
  function appliquerSauvegarde(paquet, fusion) {
    ...
    if (!fusion) { S = base; save(); return { notes: S.notes.length, favoris: S.favoris.length }; }
    ...
    if (base.serie) {
      S.serie.record = Math.max(S.serie.record || 0, base.serie.record || 0, base.serie.jours || 0);
      if (!S.serie.dernier || (base.serie.dernier || "") > S.serie.dernier) {
        S.serie.dernier = base.serie.dernier;
        S.serie.jours = base.serie.jours || S.serie.jours;
      }
    }
    ...
    save();
    return { notes: base.notes.length, favoris: base.favoris.length };
  }
