...
(function () {
    ...
    if (highlight) {
      setTimeout(function () {
        ...
        if (t) {
          ...
          catch (e) { /* certains navigateurs de test n'ont pas le défilement */ }
        }
        toggleSel(highlight);
      }, 50);
    }
  ...
  function viewPlus() {
    ...
    h += '<button class="card zarvis-card" id="zarvis-open-card" type="button">' +
      '<div class="zarvis-card-main">' +
      '<span class="zarvis-orb" aria-hidden="true"><span class="zarvis-orb-core"></span></span>' +
      '<div><div class="zarvis-card-title">ZARVIS</div>' +
      '<div class="zarvis-card-sub">' + esc(zt("subtitle")) +
      ' · ' + esc(zt("foot")) + '</div></div>' +
      '<span class="zarvis-arrow" aria-hidden="true">›</span></div></button>';
    ...
    h += '<div class="card center"><div style="font-size:1.8rem">🌿</div>' +
      ...
      '<p class="muted" style="font-size:.84rem;margin:0">Version 1.8.0 · fonctionne hors-ligne</p>' +
  ...
  // --- ZARVIS : assistant personnel hors-ligne -----------------------------
  // ZARVIS n'est pas un service distant : ses commandes texte interprètent un
  // petit vocabulaire directement dans le navigateur et s'appuient sur les
  // données déjà présentes dans l'application. La voix est une option fournie
  // par le navigateur ; le champ texte reste le mode garanti sans réseau.
  var ZARVIS_I18N = {
    fr: {
      kicker: "NOYAU LOCAL · HORS-LIGNE",
      subtitle: "Votre copilote de méditation",
      close: "Fermer ZARVIS",
      voiceOn: "Désactiver la voix de ZARVIS",
      voiceOff: "Activer la voix de ZARVIS",
      ready: "SYSTÈME PRÊT · DONNÉES LOCALES UNIQUEMENT",
      listening: "ÉCOUTE EN COURS · PARLEZ MAINTENANT",
      welcomeTitle: "Bonjour, je suis ZARVIS",
      welcome: "Un assistant biblique privé, inspiré des interfaces de science-fiction. Je peux piloter cette application, chercher dans la Bible, lire un passage et vous aider à méditer — les commandes texte fonctionnent sans Internet.",
      placeholder: "Écrivez une commande à ZARVIS…",
      microphone: "Parler à ZARVIS",
      send: "Envoyer la commande",
      foot: "Cœur local · commandes texte privées",
      quickVerse: "Verset du jour",
      quickBible: "Ouvre la Bible",
      quickPrayer: "Une prière",
      quickHelp: "Voir les commandes",
      greeting: "Bonjour. ZARVIS est prêt. Que puis-je faire pour vous ?",
      identity: "Je suis ZARVIS, votre assistant biblique local. Mon intelligence tient dans cet appareil : pas de compte, pas de serveur, pas de collecte.",
      help: "Essayez par exemple :\n• « Ouvre la Bible »\n• « Lis Jean 3:16 »\n• « Cherche berger »\n• « Mets le verset en favori »\n• « Note : prier pour ma famille »\n• « Passe en mode nuit »\n• « Calcule 12 × 4 »\n• « Minuteur 5 minutes »\n• « Donne-moi une prière »",
      fallback: "Je fonctionne hors-ligne : je comprends les commandes de la Bible et de cette application, mais je ne suis pas encore un chatbot généraliste.",
      micUnavailable: "La reconnaissance vocale dépend des capacités et des réglages de ce navigateur. Utilisez le champ de commande pour rester 100 % hors-ligne.",
      noCommand: "Je n'ai pas compris cette commande. Dites « aide » pour voir ce que je peux faire.",
      opened: "J'ouvre",
      searching: "Je recherche",
      daily: "Voici votre verset du jour",
      favoriteAdded: "Verset ajouté aux favoris.",
      favoriteRemoved: "Verset retiré des favoris.",
      noteSaved: "Note enregistrée dans votre journal.",
      prayer: "Seigneur, merci pour cette journée. Donne-moi un cœur attentif à ta Parole, la force de mettre en pratique ce que je comprends et la paix pour avancer aujourd'hui. Amen.",
      noErase: "Je ne supprime jamais vos données par une simple commande vocale. Ouvrez Réglages pour gérer vos données en toute sécurité.",
      currentVersion: "La version biblique active est",
      currentTheme: "Le thème actuel est",
      night: "nuit",
      day: "jour",
      stats: "Vous avez médité {total} jour(s), votre série actuelle est de {serie} et votre record est de {record}.",
      reminder: "J'ouvre les réglages pour configurer votre rappel quotidien."
    },
    en: {
      kicker: "LOCAL CORE · OFFLINE",
      subtitle: "Your meditation copilot",
      close: "Close ZARVIS",
      voiceOn: "Turn off ZARVIS voice",
      voiceOff: "Turn on ZARVIS voice",
      ready: "SYSTEM READY · LOCAL DATA ONLY",
      listening: "LISTENING · SPEAK NOW",
      welcomeTitle: "Hello, I am ZARVIS",
      welcome: "A private Bible assistant inspired by science-fiction interfaces. I can control this app, search the Bible, read a passage and help you meditate — text commands work without the Internet.",
      placeholder: "Type a command for ZARVIS…",
      microphone: "Talk to ZARVIS",
      send: "Send command",
      foot: "Local core · private text commands",
      quickVerse: "Today's verse",
      quickBible: "Open the Bible",
      quickPrayer: "A prayer",
      quickHelp: "Show commands",
      greeting: "Hello. ZARVIS is ready. What can I do for you?",
      identity: "I am ZARVIS, your local Bible assistant. My intelligence stays on this device: no account, no server, no tracking.",
      help: "Try for example:\n• “Open the Bible”\n• “Read John 3:16”\n• “Search shepherd”\n• “Add the verse to favourites”\n• “Note: pray for my family”\n• “Turn on night mode”\n• “Calculate 12 × 4”\n• “Timer 5 minutes”\n• “Give me a prayer”",
      fallback: "I work offline: I understand Bible and app commands, but I am not a general chatbot yet.",
      micUnavailable: "Speech recognition depends on this browser's capabilities and settings. Use the command field to stay 100% offline.",
      noCommand: "I did not understand that command. Say “help” to see what I can do.",
      opened: "Opening",
      searching: "Searching",
      daily: "Here is today's verse",
      favoriteAdded: "Verse added to favourites.",
      favoriteRemoved: "Verse removed from favourites.",
      noteSaved: "Note saved in your journal.",
      prayer: "Lord, thank you for this day. Give me an attentive heart for your Word, strength to live what I understand and peace to move forward today. Amen.",
      noErase: "I never delete your data from a simple voice command. Open Settings to manage your data safely.",
      currentVersion: "The active Bible version is",
      currentTheme: "The current theme is",
      night: "night",
      day: "day",
      stats: "You have meditated for {total} day(s), your current streak is {serie} and your record is {record}.",
      reminder: "Opening settings so you can configure your daily reminder."
    }
  };
  ...
  function zt(key) {
    var table = ZARVIS_I18N[langue] || ZARVIS_I18N.fr;
    return table[key] !== undefined ? table[key] : ZARVIS_I18N.fr[key] || key;
  }

  function zreplace(text, values) {
    return String(text).replace(/\{(\w+)\}/g, function (all, key) {
      return values[key] !== undefined ? values[key] : all;
    });
  }

  function zarvisNormaliser(text) {
  ...
  function zarvisQuickHtml() {
    var cmds = [
      [zt("quickVerse"), langue === "en" ? "What is today's verse?" : "Quel est le verset du jour ?"],
      [zt("quickBible"), langue === "en" ? "Open the Bible" : "Ouvre la Bible"],
      [zt("quickPrayer"), langue === "en" ? "Give me a prayer" : "Donne-moi une prière"],
      [zt("quickHelp"), langue === "en" ? "Help" : "Aide"]
    ];
    return '<div class="zarvis-quick">' + cmds.map(function (c) {
      return '<button type="button" data-zcommand="' + esc(c[1]) + '">' + esc(c[0]) + "</button>";
    }).join("") + "</div>";
  }

  function zarvisRenderLog() {
    var log = $("#zarvis-log");
    if (!log) return;
    var h = "";
    if (!ZARVIS.history.length) {
      h += '<div class="zarvis-welcome">' +
        '<span class="zarvis-orb zarvis-big-orb" aria-hidden="true"><span class="zarvis-orb-core"></span></span>' +
        '<h2>' + esc(zt("welcomeTitle")) + "</h2>" +
        '<p>' + esc(zt("welcome")) + "</p>" + zarvisQuickHtml() + "</div>";
    } else {
      ZARVIS.history.forEach(function (m) {
        var user = m.role === "user";
        h += '<div class="zarvis-msg ' + (user ? "user" : "zarvis") + '">' +
          '<span class="zarvis-avatar" aria-hidden="true">' + (user ? "VOUS" : "Z") + "</span>" +
          '<div class="zarvis-bubble">' + esc(m.text) +
          '<span class="zarvis-stamp">' + (user ? "VOUS" : "ZARVIS") + "</span></div></div>";
      });
      h += zarvisQuickHtml();
    }
    log.innerHTML = h;
  ...
  function zarvisUpdateControls() {
    var bg = $("#zarvis-bg");
    if (!bg) return;
    var voice = $("#zarvis-voice", bg);
    var mic = $("#zarvis-mic", bg);
    var status = $("#zarvis-status", bg);
    if (voice) {
      voice.textContent = ZARVIS.voice ? "🔊" : "🔇";
      voice.setAttribute("aria-label", ZARVIS.voice ? zt("voiceOn") : zt("voiceOff"));
      voice.setAttribute("title", ZARVIS.voice ? zt("voiceOn") : zt("voiceOff"));
      voice.classList.toggle("on", ZARVIS.voice);
    }
    if (mic) {
      var C = window.SpeechRecognition || window.webkitSpeechRecognition;
      mic.disabled = typeof C !== "function";
      mic.setAttribute("aria-label", zt("microphone"));
      mic.title = zt("microphone");
      mic.classList.toggle("listening", ZARVIS.listening);
    }
    if (status) status.textContent = ZARVIS.listening ? zt("listening") : zt("ready");
  }

  function zarvisReply(text, speakIt) {
    text = String(text || zt("noCommand"));
    ZARVIS.history.push({ role: "zarvis", text: text });
    if (ZARVIS.history.length > 40) ZARVIS.history = ZARVIS.history.slice(-40);
    zarvisRenderLog();
    if (speakIt !== false && ZARVIS.voice) zarvisSay(text);
  }

  function zarvisSay(text) {
    if (!("speechSynthesis" in window) || typeof SpeechSynthesisUtterance !== "function") return;
    var synth = window.speechSynthesis;
    var u = new SpeechSynthesisUtterance(normaliserPonctuation(text));
    u.lang = langue === "en" ? "en-US" : "fr-FR";
    u.rate = .96;
    var voices = voixTriees();
    for (var i = 0; i < voices.length; i++) {
      if (new RegExp("^" + (langue === "en" ? "en" : "fr"), "i").test(voices[i].lang || "")) {
        u.voice = voices[i];
        break;
      }
    }
    try { synth.cancel(); synth.speak(u); } catch (e) { /* voix facultative */ }
  }

  function zarvisClose() {
    var bg = $("#zarvis-bg");
    if (ZARVIS.recognition) {
      try { ZARVIS.recognition.stop(); } catch (e) { /* déjà arrêté */ }
      ZARVIS.recognition = null;
    }
    ZARVIS.listening = false;
    ZARVIS.open = false;
    if (bg) bg.remove();
    if (!$(".reader") && !$(".sheet-bg")) document.body.style.overflow = "";
  }

  function zarvisOpen() {
    if (ZARVIS.open) return;
    closeSheet();
    ZARVIS.open = true;
    var bg = el('<div class="zarvis-bg" id="zarvis-bg" role="dialog" aria-modal="true" aria-labelledby="zarvis-title">' +
      '<section class="zarvis-panel">' +
      '<header class="zarvis-head">' +
      '<button class="zarvis-close" id="zarvis-close" type="button" aria-label="' + esc(zt("close")) + '">✕</button>' +
      '<span class="zarvis-orb" aria-hidden="true"><span class="zarvis-orb-core"></span></span>' +
      '<div class="zarvis-head-copy"><div class="zarvis-kicker">' + esc(zt("kicker")) +
      '</div><div class="zarvis-title" id="zarvis-title">ZARVIS</div><div class="zarvis-subtitle">' +
      esc(zt("subtitle")) + '</div></div>' +
      '<button class="zarvis-voice on" id="zarvis-voice" type="button">🔊</button>' +
      '</header>' +
      '<div class="zarvis-scan"><span class="zarvis-scan-dot"></span><span id="zarvis-status"></span></div>' +
      '<div class="zarvis-body" id="zarvis-log"></div>' +
      '<form class="zarvis-compose" id="zarvis-compose">' +
      '<button class="zarvis-mic" id="zarvis-mic" type="button">🎙</button>' +
      '<input class="zarvis-input" id="zarvis-input" autocomplete="off" enterkeyhint="send" placeholder="' +
      esc(zt("placeholder")) + '">' +
      '<button class="zarvis-send" type="submit" aria-label="' + esc(zt("send")) + '" title="' +
      esc(zt("send")) + '">➤</button>' +
      '</form>' +
      '<div class="zarvis-foot">' + esc(zt("foot")) + "</div>" +
      '</section></div>');
    document.body.appendChild(bg);
    document.body.style.overflow = "hidden";
    zarvisRenderLog();
    zarvisUpdateControls();

    bg.addEventListener("click", function (e) {
      if (e.target === bg || e.target.closest("#zarvis-close")) { zarvisClose(); return; }
      var q = e.target.closest("[data-zcommand]");
      if (q) { zarvisCommand(q.dataset.zcommand); return; }
      if (e.target.closest("#zarvis-voice")) {
        ZARVIS.voice = !ZARVIS.voice;
        if (!ZARVIS.voice && "speechSynthesis" in window) {
          try { window.speechSynthesis.cancel(); } catch (er) { /* rien */ }
        }
        zarvisUpdateControls();
      }
      if (e.target.closest("#zarvis-mic")) zarvisListen();
    });
    $("#zarvis-compose", bg).addEventListener("submit", function (e) {
      e.preventDefault();
      var input = $("#zarvis-input", bg);
      if (!input || !input.value.trim()) return;
      var command = input.value.trim();
      input.value = "";
      zarvisCommand(command);
    });
    setTimeout(function () {
      var input = $("#zarvis-input", bg);
  ...
  function zarvisListen() {
    if (!ZARVIS.open) return;
    var C = window.SpeechRecognition || window.webkitSpeechRecognition;
    if (typeof C !== "function") { zarvisReply(zt("micUnavailable"), false); return; }
    if (ZARVIS.recognition) {
      try { ZARVIS.recognition.stop(); } catch (e) {}
      return;
    }
    var r;
    try { r = new C(); } catch (e) { zarvisReply(zt("micUnavailable"), false); return; }
    ZARVIS.recognition = r;
    r.lang = langue === "en" ? "en-US" : "fr-FR";
    r.continuous = false;
    r.interimResults = false;
    r.maxAlternatives = 1;
    r.onstart = function () {
      ZARVIS.listening = true;
      zarvisUpdateControls();
    };
    r.onresult = function (e) {
      var transcript = e && e.results && e.results[0] && e.results[0][0] && e.results[0][0].transcript;
      if (transcript) {
        var input = $("#zarvis-input");
        if (input) input.value = transcript;
        zarvisCommand(transcript);
      }
    };
    r.onerror = function () {
      ZARVIS.listening = false;
      ZARVIS.recognition = null;
      zarvisUpdateControls();
      zarvisReply(zt("micUnavailable"), false);
    };
    r.onend = function () {
      ZARVIS.listening = false;
      ZARVIS.recognition = null;
  ...
  // Cherche une référence biblique même au milieu d'une phrase (« lis Jean 3:16 »).
  function zarvisReference(raw) {
    var mots = String(raw || "")
      .replace(/[«»“”?!]/g, " ")
      .split(/\s+/)
      .filter(function (x) { return !!x; });
    for (var i = 0; i < mots.length; i++) {
      for (var j = mots.length; j > i; j--) {
        var morceau = mots.slice(i, j).join(" ").replace(/[.,;:]+$/, function (x) {
          return x === ":" ? x : "";
        });
        var p = chercherReference(morceau);
        if (p && (p.v || /\d+\s*$/.test(morceau))) return p;
      }
    }
    return null;
  }

  function zarvisRef(p) {
    return p.a + " " + p.c + (p.v ? ":" + p.v + (p.v2 && p.v2 !== p.v ? "-" + p.v2 : "") : "");
  }

  function zarvisCurrentRef() {
    var d = new Date();
    var perso = S.perso && S.perso.actif !== false ? persoRefFor(d) : null;
    return perso ? perso.ref : dailyRefFor(d);
  }

  function zarvisOpenReference(p) {
    var ref = zarvisRef(p);
    zarvisReply(zt("opened") + " " + refLabel(ref) + ".");
    setTimeout(function () {
      zarvisClose();
      openReader(p.a, p.c, p.v || null);
    }, 220);
  }

  function zarvisFindVersion(query) {
    var needle = zarvisNormaliser(query);
    if (!needle) return null;
    var found = null;
    VERSIONS.some(function (v) {
      var names = [v.id, v.nfr, v.nen];
      return names.some(function (name) {
        if (zarvisNormaliser(name).indexOf(needle) !== -1 || needle.indexOf(zarvisNormaliser(name)) !== -1) {
          found = v;
          return true;
        }
        return false;
  ...
  function zarvisFindTheme(query) {
    var needle = zarvisNormaliser(query);
    if (!needle) return null;
    return (window.THEMES || []).filter(function (item) {
      return [item.id, item.n, item.ne].some(function (value) {
        return value && (zarvisNormaliser(value).indexOf(needle) !== -1 || needle.indexOf(zarvisNormaliser(value)) !== -1);
      });
    })[0] || null;
  }

  // Petit calculateur local : on utilise une pile, jamais eval() ni un serveur.
  function zarvisCalculer(expression) {
    var expr = String(expression || "").replace(/,/g, ".").replace(/[×x]/g, "*").replace(/÷/g, "/").trim();
    if (!expr || !/^[0-9+\-*/().\s]+$/.test(expr)) return null;
    var tokens = expr.match(/\d+(?:\.\d+)?|[()+\-*/]/g);
    if (!tokens) return null;
    var valeurs = [], operateurs = [];
    var priorite = { "+": 1, "-": 1, "*": 2, "/": 2 };
    function appliquer() {
      var op = operateurs.pop();
      var b = valeurs.pop(), a = valeurs.pop();
      if (a === undefined || b === undefined || op === "/" && b === 0) return false;
      valeurs.push(op === "+" ? a + b : op === "-" ? a - b : op === "*" ? a * b : a / b);
      return true;
    }
    var precedent = "(";
    for (var i = 0; i < tokens.length; i++) {
      ...
      if (/^\d/.test(token)) {
        ...
        while (operateurs.length && operateurs[operateurs.length - 1] !== "(") {
          if (!appliquer()) return null;
        }
        if (operateurs.pop() !== "(") return null;
        precedent = "n";
      } else {
        // Le signe moins unaire est traité comme 0 - nombre.
        if (token === "-" && (precedent === "(" || precedent === "o")) valeurs.push(0);
        while (operateurs.length && operateurs[operateurs.length - 1] !== "(" &&
               priorite[operateurs[operateurs.length - 1]] >= priorite[token]) {
          if (!appliquer()) return null;
        }
        operateurs.push(token);
        precedent = "o";
    ...
    while (operateurs.length) {
      if (operateurs[operateurs.length - 1] === "(" || !appliquer()) return null;
    }
    if (valeurs.length !== 1 || !isFinite(valeurs[0])) return null;
    return Math.round(valeurs[0] * 100000000) / 100000000;
  }

  function zarvisCommand(raw) {
    raw = String(raw || "").trim();
    if (!raw || !ZARVIS.open) return;
    ZARVIS.history.push({ role: "user", text: raw });
    if (ZARVIS.history.length > 40) ZARVIS.history = ZARVIS.history.slice(-40);
    zarvisRenderLog();
    if (!BOOKS) inflate();
    var n = zarvisNormaliser(raw);
    var ref = zarvisReference(raw);

    if (/^(aide|help|commandes|que peux tu faire|what can you do)/.test(n)) {
      zarvisReply(zt("help")); return;
    }
    if (/(qui es tu|qui es-tu|ton nom|who are you|your name)/.test(n)) {
      zarvisReply(zt("identity")); return;
    }
    if (/^(bonjour|salut|hello|bonsoir|coucou|hey)\b/.test(n) && n.split(" ").length < 6) {
      zarvisReply(zt("greeting")); return;
    }
    if (/(quelle heure|quelle est l heure|what time|current time)/.test(n)) {
      zarvisReply((langue === "en" ? "It is " : "Il est ") + new Date().toLocaleTimeString(langue === "en" ? "en-US" : "fr-FR", { hour: "2-digit", minute: "2-digit" }) + ".");
      return;
    }
    if (/(quelle date|quel jour sommes nous|what date|what day)/.test(n)) {
      zarvisReply((langue === "en" ? "Today is " : "Nous sommes ") + frDate(new Date()) + ".");
      return;
    }
    if (/(comment je m appelle|quel est mon nom|what is my name|who am i)/.test(n)) {
      zarvisReply(prenom()
        ? (langue === "en" ? "You are " : "Vous êtes ") + prenom() + "."
        : (langue === "en" ? "I do not know your name yet." : "Je ne connais pas encore votre prénom."), false);
      return;
    }
    if (/(statut du systeme|diagnostic|system status|system check)/.test(n)) {
      zarvisReply((langue === "en" ? "ZARVIS // all systems nominal. Bible, journal and commands are local. " :
        "ZARVIS // tous les systèmes sont nominaux. Bible, journal et commandes sont locaux. ") +
        (BOOKS ? (langue === "en" ? "66 books ready." : "66 livres prêts.") :
          (langue === "en" ? "Bible loading." : "Bible en cours de chargement.")), false);
      return;
    }
    var calculMatch = /^(?:calcule|calculer|combien font|calculate)\s+(.+)$/i.exec(raw);
    if (calculMatch) {
      var resultat = zarvisCalculer(calculMatch[1]);
      zarvisReply(resultat === null
        ? (langue === "en" ? "I can only calculate simple arithmetic locally." : "Je peux seulement calculer une opération arithmétique simple hors-ligne.")
        : (langue === "en" ? "The result is " : "Le résultat est ") + resultat + ".", false);
      return;
    }

    // Une référence explicite a priorité sur la recherche plein texte.
    if (ref && /(lis|lire|ecoute|écoute|ouvre|montre|affiche|read|listen|open|show|cherche|recherche|search)/.test(n)) {
      zarvisOpenReference(ref); return;
    }
    if (ref && /^(?:[1-3]\s*)?[a-zà-ÿ]/i.test(raw.trim())) {
      zarvisOpenReference(ref); return;
    }

    if (/(verset du jour|verset d aujourd hui|today'?s verse|verse of the day)/.test(n)) {
      var todayRef = zarvisCurrentRef();
      var todayText = verseText(todayRef);
      zarvisReply(zt("daily") + " :\n« " + todayText + " »\n— " + refLabel(todayRef));
      return;
    }
    if (/(lis|lire|ecoute|écoute|read|listen)/.test(n) && /(verset|verse)/.test(n)) {
      var listenRef = zarvisCurrentRef();
      zarvisReply((langue === "en" ? "Here is " : "Voici ") + refLabel(listenRef) + " :\n« " +
        verseText(listenRef) + " »");
      return;
    }

    var searchMatch = /^(?:cherche|recherche|search)(?: dans (?:la )?bible| in the bible)?\s+(.+)$/i.exec(raw);
    if (searchMatch) {
      var query = searchMatch[1].trim();
      zarvisReply(zt("searching") + " « " + query + " ».", false);
      current = "bible";
      offset = 0;
      render();
      var q = $("#q");
      if (q) { q.value = query; search(query); }
      return;
    }

    var themeMatch = /(?:ouvre|affiche|montre|open|show)?\s*(?:le\s+|la\s+)?(?:theme|thème)\s+(.+)$/i.exec(raw);
    if (themeMatch && !/^(?:nuit|jour|auto|automatic)$/i.test(zarvisNormaliser(themeMatch[1]))) {
      var chosenTheme = zarvisFindTheme(themeMatch[1]);
      if (chosenTheme) {
        zarvisReply((langue === "en" ? "Opening the theme " : "J'ouvre le thème ") + loc(chosenTheme, "n") + ".", false);
        setTimeout(function () { zarvisClose(); openTheme(chosenTheme.id); }, 220);
        return;
      }
    }

    var route = null;
    if (/\b(ma meditation|ma medit|personal meditation|my meditation)\b/.test(n)) route = "perso";
    else if (/\b(plan|plans|reading plan)\b/.test(n)) route = "plans";
    else if (/\b(theme|themes|thème|thèmes)\b/.test(n)) route = "themes";
    else if (/\b(bible|explorateur|explore|scripture)\b/.test(n)) route = "bible";
    else if (/\b(journal|note|notes|diary)\b/.test(n) && !/^note\b/.test(n)) route = "journal";
    else if (/\b(progression|progress|statistiques|stats|badges)\b/.test(n)) route = "progres";
    else if (/\b(reglages|réglages|settings|plus)\b/.test(n)) route = "plus";
    else if (/\b(accueil|aujourd hui|home|today)\b/.test(n)) route = "jour";
    if (route && /\b(ouvre|affiche|montre|va|open|show|go|view|retourne|back)\b/.test(n)) {
      current = route;
      offset = 0;
      render();
      zarvisReply((langue === "en" ? "Opening " : "J'ouvre ") +
        ({ jour: "l'accueil", perso: "votre méditation", plans: "les plans", themes: "les thèmes", bible: "la Bible", journal: "le journal", progres: "votre progression", plus: "les réglages" }[route] || route) + ".", false);
      return;
    }

    if (/(verset|verse).*(favori|favorite)|favori|favorite|star|etoile|étoile/.test(n)) {
      var favRef = zarvisCurrentRef();
      var wasFav = isFav(favRef);
      toggleFav(favRef);
      zarvisReply(zt(wasFav ? "favoriteRemoved" : "favoriteAdded") + " (" + refLabel(favRef) + ").", false);
      return;
    }

    var noteMatch = /^(?:note|ecris dans mon journal|écris dans mon journal|ajoute une note|write a note)\s*(?::|que|que )?\s*(.*)$/i.exec(raw);
    if (noteMatch) {
      var noteText = noteMatch[1].trim();
      var noteRef = zarvisCurrentRef();
      if (!noteText) {
        zarvisReply((langue === "en" ? "Opening a note for " : "J'ouvre une note pour ") + refLabel(noteRef) + ".", false);
        setTimeout(function () { zarvisClose(); noteSheet(noteRef); }, 220);
      } else {
        addNote(noteRef, noteText, []);
        zarvisReply(zt("noteSaved") + " (" + refLabel(noteRef) + ").", false);
      }
      return;
    }

    var timerMatch = /(?:minuteur|timer).*?(\d+)\s*(secondes?|seconds?|sec|s|minutes?|mins?|min)?/i.exec(raw);
    if (timerMatch) {
      var amount = +timerMatch[1];
      var unit = (timerMatch[2] || "minutes").toLowerCase();
      var milliseconds = /^(?:s|sec|secs|seconde|secondes|second|seconds)$/i.test(unit)
        ? amount * 1000 : amount * 60000;
      if (amount > 0 && amount <= 1440) {
        clearTimeout(ZARVIS.timer);
        ZARVIS.timer = setTimeout(function () {
          toast((langue === "en" ? "ZARVIS timer finished." : "Minuteur ZARVIS terminé.") + " ⏱");
        }, milliseconds);
        zarvisReply((langue === "en" ? "Timer set for " : "Minuteur réglé pour ") + amount + " " + unit + ".", false);
      } else zarvisReply(langue === "en" ? "Choose a duration between 1 and 1440." : "Choisissez une durée entre 1 et 1440.", false);
      return;
    }

    if (/(mode nuit|theme nuit|dark mode|night mode|active.*nuit|turn on night)/.test(n)) {
      S.reglages.theme = "nuit"; save(); applySettings(); render();
      zarvisReply((langue === "en" ? "Night mode enabled." : "Mode nuit activé."), false); return;
    }
    if (/(mode jour|theme jour|light mode|day mode|desactive.*nuit|turn off night)/.test(n)) {
      S.reglages.theme = "jour"; save(); applySettings(); render();
      zarvisReply((langue === "en" ? "Day mode enabled." : "Mode jour activé."), false); return;
    }
    if (/(mode auto|theme auto|automatic theme|auto mode)/.test(n)) {
      S.reglages.theme = "auto"; save(); applySettings(); render();
      zarvisReply((langue === "en" ? "Automatic theme enabled." : "Thème automatique activé."), false); return;
    }

    if (/(quelle version|version actuelle|current version|which version)/.test(n) && !/change|passe|switch|use/.test(n)) {
      zarvisReply(zt("currentVersion") + " " + versionNom(version) + ".", false); return;
    }
    var versionMatch = /(?:version|traduction|translation)\s+(.+)$/i.exec(raw);
    if (versionMatch) {
      var targetVersion = zarvisFindVersion(versionMatch[1]);
      if (targetVersion) {
        changerVersion(targetVersion.id);
        zarvisReply((langue === "en" ? "Bible version changed to " : "Version biblique changée : ") + versionNom(targetVersion.id) + ".", false);
      } else zarvisReply(langue === "en" ? "I could not find that Bible version." : "Je ne trouve pas cette version biblique.", false);
      return;
    }

    if (/(donne moi une priere|donne moi une prière|une priere|une prière|pray for me|give me a prayer)/.test(n)) {
      zarvisReply(zt("prayer")); return;
    }
    if (/(ma progression|mes statistiques|how am i doing|my progress|stats)/.test(n)) {
      var st = statsProgression();
      zarvisReply(zreplace(zt("stats"), { total: st.total, serie: st.serie, record: st.record }), false); return;
    }
    if (/(rappel|reminder)/.test(n)) {
      current = "plus"; offset = 0; render();
      zarvisReply(zt("reminder"), false); return;
    }
    if (/(supprime tout|efface tout|erase everything|delete all)/.test(n)) {
      zarvisReply(zt("noErase"), false); return;
    }
    if (/(arrete la voix|stop audio|silence|be quiet)/.test(n)) {
      ZARVIS.voice = false;
      try { if ("speechSynthesis" in window) window.speechSynthesis.cancel(); } catch (e) {}
      zarvisUpdateControls();
      zarvisReply(langue === "en" ? "Voice responses muted." : "Réponses vocales désactivées.", false); return;
    }

    zarvisReply(zt("fallback") + "\n\n" + zt("noCommand"), false);
  }

  function setupZarvis() {
    var launcher = $("#zarvis-launcher");
    if (!launcher || launcher.dataset.ready) return;
    launcher.dataset.ready = "1";
    launcher.addEventListener("click", zarvisOpen);
  }

  document.addEventListener("keydown", function (e) {
    if (e.key === "Escape" && $("#zarvis-bg")) zarvisClose();
  });
  ...
  function appliquerLangueDocument() {
    ...
    var zl = $("#zarvis-launcher");
    if (zl) {
      var zlabel = langue === "en" ? "Open ZARVIS" : "Ouvrir ZARVIS";
      zl.setAttribute("aria-label", zlabel);
      zl.setAttribute("title", zlabel);
    }
  }
  ...
  document.addEventListener("click", function (e) {
    if (e.target.closest("#zarvis-open-card")) { zarvisOpen(); return; }
  ...
  window.__MB = {
    ...
    // assistant local ZARVIS
    zarvis: function () { return ZARVIS; },
    zarvisOpen: zarvisOpen,
    zarvisClose: zarvisClose,
    zarvisCommand: zarvisCommand,
    zarvisCalculate: zarvisCalculer
  };
})();
